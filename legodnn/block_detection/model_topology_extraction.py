...
class LegoDNNNode:  # LegoDNN的节点
    def __init__(self, name, type, op_type, auxiliary):
        self._name = name
        self._type = type
        self._op_type = op_type
        self._auxiliary = auxiliary  # 算子的辅助参数
    ...
    def has_input(self, node=None):
        ...
        return self.pre_nodes.get(node.get_name())  # 递归得查找前向节点的input
...
class LegoDNNGraph:  # LegoDNN的图结构
    def __init__(self):
        self.node_dict = {}
    ...
    def _add_node(self, node):  # 添加节点
        if not self.node_dict.get(node.get_name()):
            self.node_dict.update({node.get_name(): node})

    def _add_edge(self, src, des):  # 添加边
        src.add_output(des)
        des.add_input(src)

    def _find_start_node(self):  # 找到开始节点并放入列表
