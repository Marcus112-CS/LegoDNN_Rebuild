...
from legodnn.block_detection.base_block_detection_1204_new import BaseBlockDetection  # 重要基类
...
class CommonDetectionManager:
    ...
    def get_block_io_info(self, block_id):   # 获取块信息（开始节点是否为占位符、开始节点编号、终止节点编号）
        block_idx = int(block_id.split('-')[-1])
    ...
    def get_no_compressed_layers(self, block_id):  # 块中非压缩层
        block_idx = int(block_id.split('-')[-1])
        return self.block_detection.blocks_no_compressed_layers[block_idx]

    def get_blocks_start_node_name_hook(self, block_id):  # 块输入节点名
        block_idx = int(block_id.split('-')[-1])
        return self.block_detection.blocks_start_node_name_hook[block_idx]

    def get_blocks_end_node_name_hook(self, block_id):  # 块输出节点名
        block_idx = int(block_id.split('-')[-1])
