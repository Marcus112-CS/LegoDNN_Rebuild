...
import warnings
import os
import thop  # pytorch的计算量和参数统计工具
import onnx

from ...common.others import get_cur_time_str


class ModelSaveMethod(enum.Enum):  # 权重/全模型/保存到磁盘（方便快速推理）/ONNX
    """
...
def save_model(model: torch.nn.Module,        # 4种格式进行模型保存
               model_file_path: str,
    ...
    model.eval()  # 评估模式,减少不必要的模型前向传播问题（对应model.train()）
    if save_method == ModelSaveMethod.WEIGHT:
        ...
        dummy_input = torch.ones(model_input_size, device=get_model_device(model))  # 全1输入矩阵
...
def get_model_size(model: torch.nn.Module, return_MB=False):  # 全量保存模型后按MB测量
...
def get_model_latency(model: torch.nn.Module, model_input_size: Tuple[int], sample_num: int,   # 考虑GPU warmup和异步并行性
...
def get_model_flops_and_params(model: torch.nn.Module, model_input_size: Tuple[int]):
    ...
    """
    device = get_model_device(model)  # 获取设备名
