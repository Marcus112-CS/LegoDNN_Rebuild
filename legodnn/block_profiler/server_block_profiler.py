...
from ..pure_runtime import PureRuntime
...
class ServerBlockProfiler:
    ...
    def _analysis_composed_models_acc(self, blocks_sparsities):
        ...
        for blocks_sparsity in pbar:
            model_str = self._blocks_sparsity_to_strkey(blocks_sparsity) # 获取输出信息
            # print(model_str)
    ...
    def _generate_envir_sparsities(self):
        ...
        for j in range(max_block_sparsity_len): # 块稀疏个数
            tmp = []
            for k in range(block_num): # 块的个数
    ...
    def profile_all_compressed_blocks(self):
        ...
        blocks_sparsities = []
        # 对于每个块
        for block_index in range(blocks_num): # 对于每个块
            # 这个块的一个稀疏度
            for sparsity in self._block_manager.get_blocks_sparsity()[block_index]: # 对于每一个稀疏度
                # 对于每个 块稀疏度列表
                for envir_sparsity in envir_sparsities: # 对于每一种环境
