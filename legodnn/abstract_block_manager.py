...
from typing import List, Any, Tuple  # 提升代码可读性（python>3.5）

from .abstract_model_manager import AbstractModelManager


class AbstractBlockManager(abc.ABC):  # LegoDNN block所有属性
    ...
    def __init__(self, blocks_id: List[str], blocks_sparsity: List[List[float]], model_manager: AbstractModelManager):
        ...
        self._blocks_sparsity = blocks_sparsity  # sparsity
        self._model_manager = model_manager  # 所属的模型manager
    ...
    @abc.abstractmethod
    def get_pruned_block(self, model: torch.nn.Module, block_id: str, block_sparsity: float,  # 块减枝
    ...
    @abc.abstractmethod
    def get_time_profilers_of_all_blocks(self, model: torch.nn.Module, device: str):  # time profile ...
